function [VIF_met, VIF_max, sig_all, P_max ,search_metric]=test_combo_matlabOpt(X,y,pct, VIFthresh, nseries, search_metric_flag, VIF_stop_flag)
...
if VIF_max<=VIFthresh
    ...
    VIF_met=false;
    if VIF_stop_flag>0 %If haulting calculations for VIF> VIFthresh
        sig_all = false;
        P_max = 0;
        search_metric= Inf;
        return
...
if all(sig(1:size(sig,1)-nseries))
    ...
    sig_all=false;
     if VIF_stop_flag==2 %If haulting calculations for VIF> VIFthresh
        search_metric= Inf;
        return
    end
end
